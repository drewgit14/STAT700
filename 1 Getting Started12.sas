...
\brief		brief description
			...
			An analysis of mean Calories Per Serving using data
			from the years  1936,1946,1961,1963,1975,1997, and 2006
...
data PlotCookingTooMuch;
  set CookingTooMuch;
  CaloriesPerServing = CaloriesPerServingMean; /*changed from CaloriesPerRecipe=CaloriesPerRecipeMean to CaloriesPerServing=CaloriesPerServingMean*/
  Lower = CaloriesPerServing-%ConfidenceInterval(CaloriesPerServingSD,&n); /*Changed from CaloriesPerRecipe to CaloriesPerServing */
  Upper = CaloriesPerServing+%ConfidenceInterval(CaloriesPerServingSD,&n);/*Changed from CaloriesPerRecipe to CaloriesPerServing */
  keep Year CaloriesPerServing Lower Upper; /*Changed from CaloriesPerRecipe to CaloriesPerServing */
run;
...
*/
/* 2018-05-29 - Changed confidence interval values (ReferenceValues) from [1638.7,2608.9,2368.7,3743.7] to [214.4,325.8,359.1,514.7] */
proc iml;
...
proc sgplot data=PlotCookingTooMuch noautolegend;                                                                                                  
   scatter x=Year y=CaloriesPerServing / yerrorlower=Lower       /*Changed from CaloriesPerRecipe to CaloriesPerServing */                                                                                     
                           yerrorupper=Upper                                                                                            
                           markerattrs=(color=blue symbol=CircleFilled);                                                                
   series x=Year y=CaloriesPerServing / lineattrs=(color=blue pattern=2); /*Changed from CaloriesPerRecipe to CaloriesPerServing */
   yaxis label= "Calories";
   title1 'Calories per Serving';    /*Changed from Calories per Recipe to Calories per Serving*/                                                               
run; 
