...
Use the `fastest.csv` file from Homework 5, read this into a data table. Plot each combination of columns (excluding `Make` and `Model`) with `MPH` as the dependent variable.

## Part a.

Create a list of independent variable names by excluding 'MPH' (this will be our independent variable), 'Make' and 'Model' (there are too few observations for many of these entries) from a vector containing the column names from `fastest.csv`
