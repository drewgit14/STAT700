...
\brief		brief description
			...
			Exercise 3 for Homework assignment;

\version	\$Revision: 1.0 $
\author		\$Author: Andrew Marshall $
\date		\$Date:	2018-07-08 $
...
title 'Exercise 3';
proc iml;
/*generate sequence for vector1*/
vector1 = 0:9;
/*mean function for vector*/
  start vectormean(vectorinput);
  vectmean = mean(vectorinput[,:]);
   return(vectmean);
   finish vectormean;
print(vectormean(vector1));
quit;
